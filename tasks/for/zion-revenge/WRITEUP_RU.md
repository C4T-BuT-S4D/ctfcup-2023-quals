...
Проверяе, что длина `handshake` действительно 3612:
```python
...
Если вспомнить dlv, то можно понять, что исопльзуется grpc. И можно предположить, что траффик как-то сжат или закодирован.
